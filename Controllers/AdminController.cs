      ...
      //  [HttpPost]
       // [ActionName("RegisterExternal")]
        public async Task<IActionResult> RegisterExternalConfirmed(ExternalLoginViewModel model)
        {
            ...
            if (info == null)
            {
                await Login(new LoginViewModel { UserName = model.UserName, Password = "123456", ReturnUrl = model.ReturnUrl });
               // return RedirectToAction("Login", new LoginViewModel { UserName=model.UserName, Password="123456", ReturnUrl = model.ReturnUrl});
                //  return RedirectToAction("Login");
            }
            ...
            if (result.Succeeded)
            {
                ...
                if (claimsResult.Succeeded)
                {
                    ...
                    if (identityResult.Succeeded)
                    {
                        ...
                        return RedirectToAction("Privacy", "Home", model);
                        //return RedirectToAction("Index");
                    }
                }
            }
